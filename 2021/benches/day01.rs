...
fn bench_part2_variants(c: &mut Criterion) {
    let input = read_input("inputs/day01_p1.txt");

    let mut group = c.benchmark_group("day01_part2");
    group.bench_function("part2_windows3_sum", |b| {
        b.iter(|| part2(black_box(&input)))
    });
    group.bench_function("part2_windows4_compare", |b| {
        b.iter(|| part2_win4(black_box(&input)))
    });
    group.finish();
}

criterion_group!(benches, bench_part2_variants);
